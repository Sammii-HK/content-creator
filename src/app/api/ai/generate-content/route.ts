...
const ContentGenerationRequestSchema = z.object({
  ...
  personaId: z.string(),
  templateType: z
    .enum(['instagram-reel', 'youtube-short', 'tiktok-video', 'twitter-video'])
    .optional(),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { videoId, personaId, templateType, prompt, customRequirements } =
    ...
    if (!templateType) {
      ...
      const recommendations = await aiTemplateMatcher.analyzeSegmentsForTemplates(segments, {
        name: video.name,
    ...
    const contentPlan = {
      ...
      segments: selectedSegments.map((s) => ({
        ...
        startTime: s.startTime,
        endTime: s.endTime,
        adjustedStartTime: s.startTime,
        adjustedEndTime: s.endTime,
    ...
    return NextResponse.json({
      ...
      metadata: {
        ...
        averageQuality:
          selectedSegments.reduce((acc, s) => acc + s.quality, 0) / selectedSegments.length,
